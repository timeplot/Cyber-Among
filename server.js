...
io.on('connection', (socket) => {
  ...
  socket.on('start-game', () => {
    ...
    if (players.length >= 1) { // Can start with just 1 player for testing
      ...
      players.forEach((player, index) => {
        ...
        player.vote = null;
        // Reset task progress for new game
        player.taskProgress = {
          tasksCompleted: 0,
          task1: { completed: false, data: null },
          task2: { completed: false, data: null },
      ...
      players.forEach(player => {
        ...
        if (playerSocket) {
          ...
          // Restore task progress if exists (for reconnects)
  ...
  socket.on('save-task-progress', (data) => {
    ...
    if (player && player.role === 'crewmate') {
      ...
      // Check if all tasks completed for this player
  ...
  socket.on('attempt-sabotage', (data) => {
    ...
    if (impostor && impostor.role === 'impostor' && targetPlayer && targetPlayer.role === 'crewmate') {
      ...
      const cooldownRemaining = 30000 - (Date.now() - lastKillTime); // 30 second cooldown
      
      if (cooldownRemaining > 0 && targetPlayer.sabotageCount >= 2) {
        // About to kill but on cooldown
  ...
  // Trigger sabotage (for task disruption)
  socket.on('trigger-sabotage', (data) => {
    const impostor = gameState.players.get(socket.id);
    
    if (impostor && impostor.role === 'impostor') {
      // Broadcast sabotage to all crewmates
  ...
  function startEmergencyMeeting() {
    ...
    // Auto-end meeting after duration
    setTimeout(() => {
      if (gameState.players.size > 0) {
        processVotes();
      }
...
server.listen(PORT, () => {
  ...
  console.log(`👥 Players can join and start immediately!`);
});